...
Processed and labeled thousands of comments.
Achieved classification accuracy up to 80%.
Extracted targeted patterns (e.g., feature requests) to guide product and content decisions.
Technologies Google YouTube Data API, Python, scikit-learn (Random Forest), pandas, regex, (optionally) Jupyter, and plotting libraries for visualization.
...
Engineered an end-to-end pipeline using the Google YouTube Data API to collect and preprocess thousands of comments; developed a Random Forest sentiment classifier (up to 80% accuracy) and REGEX-based pattern extraction to identify feature requests and produce actionable, category-level insights for product/content prioritization.
